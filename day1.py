...
'Age':[22,25,26,27],
'Occupation':['Engineer','Artist','Chef','Doctor']})
st.dataframe(df)

st.subheader('Edible_Dataframe')
editor=st.data_editor(df)
print(editor)

st.subheader('Static_Table')
static=st.table(df)
...
with st.form(key='sample form'):
    ...
    slider_value=st.select_slider('Select a range',options=range(1,10))
